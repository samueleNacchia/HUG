...
% ------------------------------------------
    ...
    \newpage
    \section{Conclusioni}
    L'analisi condotta attraverso la preparation e i due algoritmi di classificazione
    ha permesso di identificare i fattori più influenti nella previsione della depressione studentesca e di selezionare il modello più performante.
    ...
    L'analisi comparativa dei modelli, riassunta nel grafico di performance, evidenzia che la combinazione
    \textbf{Logistic Regression} offre le prestazioni migliori in assoluto,
    ...
    Alla luce dei risultati ottenuti, il modello selezionato per la fase di deployment è la \textbf{Logistic Regression}
    addestrata sui dati processati.
    ...
    \begin{table}[h!]
        \centering
        \caption{Top 5 Feature per impatto sulla predizione (Logistic Regression)}
    ...
    Per valutare gli errori commessi dal modello, riportiamo la Matrice di Confusione.
    Essa mostra la distribuzione tra predizioni corrette (sulla diagonale principale)
    ed errori di classificazione.


    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.6\textwidth]{images/graphs/cmatrix}
        \label{fig:conf_matrix}
    \end{figure}


    Il modello dimostra una solida capacità di distinguere le classi, minimizzando i Falsi Positivi
    (alta Precision), aspetto cruciale per evitare di segnalare erroneamente come a rischio studenti che non
    lo sono.
