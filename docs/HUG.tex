...
% -------------------------------------------------------------------------
    ...
    Sebbene il sistema rappresenti un solido punto di partenza per il monitoraggio preventivo,
    sono stati identificati diversi percorsi di evoluzione per potenziarne l'efficacia e l'integrazione nel contesto accademico reale.

    \subsection{Integrazione con i Servizi di Ateneo}
    Il principale sviluppo futuro riguarda la transizione da un sistema di pura analisi a un ecosistema di \textbf{intervento integrato}:

    \begin{itemize}
        \item \textbf{Sistema di Alerting}: Al superamento di una determinata soglia critica,
        il sistema potrebbe generare una segnalazione automatica indirizzata a un team di esperti (psicologi o tutor specializzati).
        \item \textbf{Contatto Proattivo}: In presenza di un rischio elevato, l'esperto potrebbe contattare personalmente lo studente,
        abbattendo la barriera dell'autosegnalazione che spesso frena chi si trova in uno stato di profondo disagio.
        \item \textbf{Percorsi di Supporto Personalizzati}: Sulla base delle feature che hanno contribuito maggiormente al punteggio di rischio,
        il sistema potrebbe suggerire allo studente risorse specifiche, come corsi sulla gestione del tempo e dell'ansia da esame.
    \end{itemize}

    \subsection{Evoluzione Tecnica e Contestuale}
    Per superare i limiti attuali legati alla natura del dataset, si prevedono i seguenti step evolutivi:

    \begin{itemize}
        \item \textbf{Localizzazione e Validazione}: Avviare una raccolta dati specifica su campioni di studenti italiani per calibrare i coefficienti del modello sulle dinamiche socio-culturali.
        \item \textbf{Monitoraggio Longitudinale}: Evolvere l'architettura dell'agente da episodica a sequenziale.
        Analizzando come le risposte di uno studente cambiano nel tempo, il sistema potrebbe identificare trend di peggioramento prima che diventino critici.
        \item \textbf{Integrazione di Feature Passive}: Valutare l'integrazione di dati oggettivi, come la frequenza di accesso alle piattaforme di e-learning.
    \end{itemize}

    In conclusione, \textit{HUG} non si propone di sostituire il professionista,
    ma di agire come un radar capace di illuminare zone d'ombra dove il disagio spesso rimane invisibile.
    L'obiettivo finale rimane la creazione di un ambiente universitario più attento e solidale,
    dove nessuno studente debba affrontare le proprie difficoltà in isolamento.
